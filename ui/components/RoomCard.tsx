...
export function RoomCard({
  ...
  return (
    <article
      className={`rounded-xl border border-slate-200 dark:border-slate-800 shadow p-4
    ...
    >
      ...
      {topic && <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{topic}</p>}

      <div className="mt-3 flex items-center justify-between">
        <p className="text-sm text-slate-500">
          Utenti online:{' '}
          <strong className="text-slate-700 dark:text-slate-200">{membersOnline}</strong>
        </p>
        <button
