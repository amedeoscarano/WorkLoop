...
export function PresenceBar({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    ch.subscribe((status: any) => { if (status === 'SUBSCRIBED') ch.track({ id: me.id, name: me.name }) })
    return () => { ch.untrack(); ch.unsubscribe() }
  }, [roomId, me.id, me.name])
  return (
    <div className="flex -space-x-2" role="list" aria-label="Utenti presenti">
      {users.map(u => (
        <a key={u.id} role="listitem" className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-slate-200 text-xs hover:ring-2 ring-blue-500" href={u.id!==me.id? `/dm/dm_${u.id}` : undefined} title={u.name} aria-label={`Utente ${u.name}${u.id!==me.id? ', apri DM' : ''}`}>
