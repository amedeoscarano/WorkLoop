...
export function SidebarNav(){
  ...
  return (
    <nav aria-label="Navigazione" className="hidden lg:flex flex-col items-center w-12 py-3 rounded-xl bg-indigo-600 text-white">
      {items.map(item => {
        const active = pathname === item.href
        return (
          <Link key={item.href} href={item.href as any} aria-label={item.label} aria-current={active? 'page' : undefined}
